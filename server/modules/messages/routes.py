...
@_(bp, '/messages/<user_id>', methods=['POST'])
async def send_message_route(request, user_id):
    logger.info('Received POST request on /messages/%s', user_id)
    result, status_code = await message.send_message(request, user_id)
